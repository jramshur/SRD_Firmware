...
{   
	...
	LCD_1_Position(0,0);
	LCD_1_PrintString("SRD Diagnostics");
	...
	if (flagSPI_Pass && flagSetup_Pass) // both test pass
	{
		...
		LCD_1_Position(1,0);
		LCD_1_PrintString("INTAN All OK");
	} 
	else if (~flagSPI_Pass)	// SPI Failed
	{
		...
		LCD_1_Position(1,0);
		LCD_1_PrintString("INTAN SPI Fail");
	}
	else if (~flagSetup_Pass) // Setup only failed
	{
		...
		PWM_1_WritePeriod(255);
		LCD_1_Position(1,0);
		LCD_1_PrintString("INTAN Setup Fail");
	}
	else // all test fail
	{
		...
		PWM_1_WritePeriod(0);	
		LCD_1_Position(1,0);
		LCD_1_PrintString("INTAN All Fail");
	}		
