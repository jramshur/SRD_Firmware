...
void SwClock_1_Start(void) 
{
    ...
    SwClock_1_CLKEN |= SwClock_1_CLKEN_MASK;
	SwClock_1_CLKSTBY |= SwClock_1_CLKSTBY_MASK;
}
...
void SwClock_1_Stop(void) 
{
    ...
    SwClock_1_CLKEN &= (uint8)(~SwClock_1_CLKEN_MASK);
	SwClock_1_CLKSTBY &= (uint8)(~SwClock_1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SwClock_1_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SwClock_1_StopBlock(void) 
{
    if ((SwClock_1_CLKEN & SwClock_1_CLKEN_MASK) != 0u)
    {
...
#if defined(SwClock_1__CFG3)
        CLK_DIST_AMASK = SwClock_1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SwClock_1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SwClock_1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SwClock_1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SwClock_1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SwClock_1_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SwClock_1_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        SwClock_1_CLKSTBY &= (uint8)(~SwClock_1_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: SwClock_1_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SwClock_1_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = SwClock_1_GetSourceRegister();
    uint16 oldDivider = SwClock_1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SwClock_1_CLKEN & SwClock_1_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SwClock_1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(SwClock_1_DIV_PTR);
}


/*******************************************************************************
* Function Name: SwClock_1_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SwClock_1_SetModeRegister(uint8 modeBitMask) 
{
    SwClock_1_MOD_SRC |= modeBitMask & (uint8)SwClock_1_MODE_MASK;
}


/*******************************************************************************
* Function Name: SwClock_1_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SwClock_1_ClearModeRegister(uint8 modeBitMask) 
{
    SwClock_1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SwClock_1_MODE_MASK));
}


/*******************************************************************************
* Function Name: SwClock_1_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 SwClock_1_GetModeRegister(void) 
{
    return SwClock_1_MOD_SRC & (uint8)(SwClock_1_MODE_MASK);
}


/*******************************************************************************
* Function Name: SwClock_1_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SwClock_1_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = SwClock_1_GetDividerRegister();
    uint8 oldSrc = SwClock_1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SwClock_1_MOD_SRC |= CYCLK_SSS;
        SwClock_1_MOD_SRC =
            (SwClock_1_MOD_SRC & (uint8)(~SwClock_1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SwClock_1_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 SwClock_1_GetSourceRegister(void) 
{
    return SwClock_1_MOD_SRC & SwClock_1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SwClock_1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SwClock_1_SetPhaseRegister(uint8 clkPhase) 
{
    SwClock_1_PHASE = clkPhase & SwClock_1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SwClock_1_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 SwClock_1_GetPhaseRegister(void) 
{
    return SwClock_1_PHASE & SwClock_1_PHASE_MASK;
}
